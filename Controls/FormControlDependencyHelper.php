...
// <editor-fold defaultstate="collapsed" desc="constructor">
	...
	function __construct(BaseControl $control, $controlClass = "dependentControl")
	{
		...
		if ($this->control->lookup("Nette\Forms\Container", false) === null) throw new InvalidArgumentException("Components should be assigned to FormContainer !");
	}
...
// <editor-fold defaultstate="collapsed" desc="getters & setters & add & remove">
	...
	{
		$this->createButton();
		if (!is_callable($callback)) throw new InvalidArgumentException("Not callable !");
	...
	{
		if ($need == true) $this->createButton();
		return $this->button;
	}
...
// <editor-fold defaultstate="collapsed" desc="tools & helpers">
	...
	public function createButton()
	{
		...
		// create button
		if ($this->buttonText === null) throw new InvalidArgumentException("Null caption of button is crappy *** !!! (ajax not working properly)");
		...
		switch ($this->buttonPosition) {
			...
			case(self::POSITION_AFTER_CONTROL):
				...
				foreach ($components as $name => $component) {
					...
					if ($name == $findName) $found = true;
				}
				if ($nextName === null) $container->addComponent($this->button, $buttonName); else
	...
	protected function updateControlClass()
	{
		if ($this->controlClass !== null) {
			...
			if (!in_array($this->controlClass, $classes)) $this->control->getControlPrototype()->class($this->controlClass);
		}
	}
